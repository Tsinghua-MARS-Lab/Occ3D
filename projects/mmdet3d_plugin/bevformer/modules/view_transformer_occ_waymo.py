...
class LSSViewTransformerOccWaymo(BaseModule):
    ...
    def __init__(
        ...
        super(LSSViewTransformerOccWaymo, self).__init__()
    ...
    def get_ego_coor(self, pose_metas):
        ...
        lidar2img, sensor2ego, cam_intrinsic=pose_metas
        B, N,_,_=lidar2img.shape
        D,H,W,_=self.frustum.shape
        points = torch.cat(
            (self.frustum, torch.ones_like(self.frustum[..., :1])), -1).to(sensor2ego)
        ...
        sensor2ego=sensor2ego.view(B,N,1,1,1,4,4).repeat(1,1,D,H,W,1,1)
        ...
        points= (sensor2ego @ points).squeeze(-1)
