...
class BEVFormerEncoder3DConv(TransformerLayerSequence):
    ...
    def get_reference_points(H, W, Z=8, n_p_in_voxel=2,n_voxel=8, dim='3d', bs=1, device='cuda', dtype=torch.float):
        ...
        if dim == '3d':
            zs = torch.linspace(0.2, Z - 0.2, n_p_in_pillar, dtype=dtype,
                                device=device).view(n_voxel,n_p_in_voxel, 1, 1).permute(1,0,2,3).expand(n_p_in_voxel,n_voxel, H, W)  / Z
        ...
        # reference points on 2D bev plane, used in temporal self-attention (TSA).
            ...
            ref_2d = torch.stack((ref_x, ref_y), -1).unsqueeze(0).repeat(n_voxel,1,1).permute(2,0,1).flatten(1).permute(1,0)
            ref_2d = ref_2d.unsqueeze(0).repeat(bs,1, 1,).unsqueeze(2)
    ...
    def forward(self,
        ...
        for lid, layer in enumerate(self.layers):
            ...
            bs, _, c = output.shape
            output = output.permute(0, 2, 1).reshape(bs, c, self.bev_z, bev_h, bev_w)
