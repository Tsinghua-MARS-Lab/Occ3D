...
CLASS_NAMES = [
    'GO',
    'TYPE_VEHICLE', "TYPE_PEDESTRIAN", "TYPE_SIGN", "TYPE_BICYCLIST", 
...
class Metric_mIoU():
    def __init__(self,
        ...
        self.point_cloud_range = [-40, -40, -5., 40, 40, 7.8]
    ...
    def get_dicts(self, ):
        ...
        self.pred_path = dirs_list[2] # "work_dirs/bevformer_base_occ_conv3d_waymo_ambiguous/results_epoch6/"
        ...
        preds_dict = {}
        for file in os.listdir(self.pred_path)[::self.load_interval]:
    ...
    def __call__(self):
        ...
        for scene_token in tqdm(preds_dict.keys()):
            ...
            try:
                ...
                semantics_pred = pred['output'][0]
                semantics_gt, mask_lidar, mask_camera = pred['voxel_semantics'], pred['mask_lidar'], pred['mask_camera']
                if len(semantics_gt.shape) == 4: semantics_gt = semantics_gt[0]
                if len(mask_lidar.shape) == 4: mask_lidar = mask_lidar[0]
                if len(mask_camera.shape) == 4: mask_camera = mask_camera[0]
                # remap
                ...
                for idx in np.unique(masked_semantics_gt):
                    if idx not in voxel_cout_gt:
                        voxel_cout_gt[idx] = 0
                    voxel_cout_gt[idx] += (masked_semantics_gt==idx).sum()
                for idx in np.unique(masked_semantics_pred):
                    if idx not in voxel_cout_pred:
                        voxel_cout_pred[idx] = 0
                    voxel_cout_pred[idx] += (masked_semantics_pred==idx).sum()                
        ...
        keysList = list(voxel_cout_gt.keys())
        keysList.sort()
        for key in keysList:
            class_name = CLASS_NAMES[key]
            if key not in voxel_cout_gt: voxel_cout_gt[key] = 0
            if key not in voxel_cout_pred: voxel_cout_pred[key] = 0
            print('     class: {},      gt cout: {},    pred cout: {}'.format(class_name, voxel_cout_gt[key], voxel_cout_pred[key]))
        return mIoU
...
class Metric_F1Score():
    ...
    def get_dicts(self, ):
        ...
        preds_dict = {}
        for file in os.listdir(pred_path)[::self.load_interval]:
            scene_token = file.split('.npz')[0]
            preds_dict[scene_token] = os.path.join(self.pred_path, file)
        print('number of pred samples = {}'.format(len(preds_dict)))
        return gts_dict, preds_dict
